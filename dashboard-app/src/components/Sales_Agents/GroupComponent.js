...
const GroupComponent = ({ className = "" }) => {
  ...
  useEffect(() => {
    fetch('http://localhost:8000/api/sales-agents')
      ...
      .then(data => {
        const activeSalesAgents = data.filter(agent => agent.active === "No");
  ...
  const getcampiagnNameById = (id) => {
    const campiagn = campaigns.find(campiagn => campiagn.id == id);
    return campiagn ? campiagn.name : "Unknown";
  };
      ...
      <tbody>
        {agents.map((agent) => (
          <tr key={agent.id}>
            ...
            <td className="border border-gray-600 px-4 py-2">{getcampiagnNameById(agent.campaign)}</td>
      ...
      <div className="absolute top-[24px] left-[40px] w-[869.3px] flex flex-col items-start justify-start gap-[18px] max-w-full text-left text-xl">
        ...
        <div className="self-stretch flex flex-row items-start justify-between max-w-full gap-[20px] mq1050:flex-wrap">
          <button
            ...
            className={`cursor-pointer [border:none] pt-[15px] px-6 pb-3.5 ${teamType === "teams" ? "bg-[#067457]" : "bg-green-600"
          ...
          <button
            ...
            className={`cursor-pointer [border:none] pt-[15px] px-6 pb-3.5 ${teamType === "BDR" ? "bg-[#067457]" : "bg-green-600"
              } rounded-lg flex flex-row items-start justify-start whitespace-nowrap z-[3] hover:bg-green-300`}
          >
            <div className="h-[29px] w-[120.1px] relative rounded-4xs bg-seagreen hidden" />
            <b className="h-0 relative text-mini leading-[0%] font-bold font-nunito text-white text-left inline-block min-w-[72px] z-[1]">
          ...
          <button
            ...
            className={`cursor-pointer [border:none] pt-[15px] px-6 pb-3.5 ${teamType === "Vetting" ? "bg-[#067457]" : "bg-green-600"
              } rounded-lg flex flex-row items-start justify-start whitespace-nowrap z-[3] hover:bg-green-300`}
          >
            <div className="h-[29px] w-[120.1px] relative rounded-4xs bg-seagreen hidden" />
            <b className="h-0 relative text-mini leading-[0%] font-bold font-nunito text-white text-left inline-block min-w-[72px] z-[1]">
          ...
          <button
            ...
            className={`cursor-pointer [border:none] pt-[15px] px-6 pb-3.5 ${teamType === "Inbound" ? "bg-[#067457]" : "bg-green-600"
              } rounded-lg flex flex-row items-start justify-start whitespace-nowrap z-[3] hover:bg-green-300`}
          >
            <div className="h-[29px] w-[120.1px] relative rounded-4xs bg-seagreen hidden" />
            <b className="h-0 relative text-mini leading-[0%] font-bold font-nunito text-white text-left inline-block min-w-[72px] z-[1]">
          ...
          <button
            ...
            className={`cursor-pointer [border:none] pt-[15px] px-6 pb-3.5 ${teamType === "Outbound" ? "bg-[#067457]" : "bg-green-600"
              } rounded-lg flex flex-row items-start justify-start whitespace-nowrap z-[3] hover:bg-green-300`}
          >
            <div className="h-[29px] w-[120.1px] relative rounded-4xs bg-seagreen hidden" />
            <b className="h-0 relative text-mini leading-[0%] font-bold font-nunito text-white text-left inline-block min-w-[72px] z-[1]">
